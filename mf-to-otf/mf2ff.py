    ...
    def process_commands(self, start_time_ff, cmds):
        ...
        while i < len(cmds):
            ...
            # TODO This section needs extensive testing and rework!
                ...
                #
                # This elif block has been modified from the original by DY
                # to avoid crashing over the ':' parsing.  The underlieing
                # cause of hte issue is not known and this work-around may
                # introduce more problems.
                #
                cmd_body_parts = self.cmd_body.split('>> ')
                # print( "BODY: " + self.cmd_body )
                hppp = float(cmd_body_parts[0])
                first_fontdimen = 0
                # print("cmd_body_parts[1]: " + cmd_body_parts[1] )
                if( cmd_body_parts[1] != ':' ):
                    first_fontdimen = int(cmd_body_parts[1].strip(':'))
                    params = [float(p.strip(':')) for p in cmds[i+1][2].split('>> ')]
                    i += 1
